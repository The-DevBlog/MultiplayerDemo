...
public partial class NavGrid : Node
{
	...
	public void BuildField(List<Vector2I> startPositions, Vector2I targetPos)
	{
		...
		BuildFlowField(resolvedAllowedSectors);
		_debugRenderer.SetColors(GridColor, BlockedColor, FlowColor, TargetColor, SectorColor, PortalColor);
	...
	private void BuildFlowField(HashSet<Vector2I> allowedSectors)
	{
		foreach (NavCell cell in _cells)
		{
			NavSector sector = GetSectorForCell(cell.Position);
			if (sector == null || !allowedSectors.Contains(sector.Position))
			{
